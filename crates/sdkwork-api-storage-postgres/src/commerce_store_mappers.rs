...
impl PostgresAdminStore {
    pub(crate) fn map_postgres_commerce_order_row(row: PgRow) -> Result<CommerceOrderRecord> {
    ...
    pub(crate) fn map_postgres_commerce_payment_event_row(
        row: PgRow,
    ...
    pub(crate) fn map_postgres_payment_method_row(row: PgRow) -> Result<PaymentMethodRecord> {
    ...
    pub(crate) fn map_postgres_payment_method_credential_binding_row(
        row: PgRow,
    ...
    pub(crate) fn map_postgres_commerce_payment_attempt_row(
        row: PgRow,
    ...
    pub(crate) fn map_postgres_commerce_webhook_inbox_row(
        row: PgRow,
    ...
    pub(crate) fn map_postgres_commerce_webhook_delivery_attempt_row(
        row: PgRow,
        ...
        Ok(CommerceWebhookDeliveryAttemptRecord {
            ...
            processing_status: row.try_get::<String, _>("processing_status")?,
            response_code: row
                .try_get::<Option<i32>, _>("response_code")?
                .map(u16::try_from)
                .transpose()?,
            error_message: row.try_get::<Option<String>, _>("error_message")?,
    ...
    pub(crate) fn map_postgres_commerce_refund_row(row: PgRow) -> Result<CommerceRefundRecord> {
    ...
    pub(crate) fn map_postgres_commerce_reconciliation_run_row(
        row: PgRow,
    ...
    pub(crate) fn map_postgres_commerce_reconciliation_item_row(
        row: PgRow,
