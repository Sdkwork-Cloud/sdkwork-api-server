...
#[tokio::test]
async fn evals_route_returns_invalid_request_without_provider() {
    ...
    let response = app
        ...
        .oneshot(
            Request::builder()
                ...
                .body(Body::from(
                    "{\"name\":\"qa-benchmark\",\"data_source_config\":{\"type\":\"file\",\"file_id\":\"file_local_1\"}}",
                ))
                .unwrap(),
        )
        .await
    ...
    assert_openai_invalid_request(
        response,
        "Local eval fallback is not supported without an upstream provider.",
...
#[tokio::test]
async fn evals_list_route_returns_invalid_request_without_provider() {
    let app = sdkwork_api_interface_http::gateway_router();
    let response = app
        .clone()
        .oneshot(
    ...
    assert_openai_invalid_request(
        response,
        "Local eval listing fallback is not supported without an upstream provider.",
        "invalid_eval_request",
    )
    .await;
}
...
#[tokio::test]
async fn eval_retrieve_route_returns_not_found_without_local_state() {
    let app = sdkwork_api_interface_http::gateway_router();
    let response = app
        .clone()
        .oneshot(
...
#[tokio::test]
async fn eval_update_route_returns_not_found_without_local_state() {
    let app = sdkwork_api_interface_http::gateway_router();
    let response = app
        .clone()
        .oneshot(
...
#[tokio::test]
async fn eval_delete_route_returns_not_found_without_local_state() {
    let app = sdkwork_api_interface_http::gateway_router();
    let response = app
        .clone()
        .oneshot(
...
#[tokio::test]
async fn eval_runs_list_route_returns_invalid_request_without_local_run_state() {
    let app = sdkwork_api_interface_http::gateway_router();
    let response = app
        .clone()
        .oneshot(
            Request::builder()
                .method("GET")
                .uri("/v1/evals/eval_local_1/runs")
                .body(Body::empty())
                .unwrap(),
        )
        .await
    ...
    assert_openai_invalid_request(
        response,
        "Persisted local eval run state is required for local run listing.",
        "invalid_eval_request",
    )
    .await;
}
...
#[tokio::test]
async fn eval_runs_create_route_returns_invalid_request_without_local_run_state() {
    let app = sdkwork_api_interface_http::gateway_router();
    let response = app
        .clone()
        .oneshot(
    ...
    assert_openai_invalid_request(
        response,
        "Persisted local eval run state is required for local run creation.",
        "invalid_eval_request",
    )
    .await;
}
...
#[tokio::test]
async fn eval_run_retrieve_route_returns_not_found_without_local_state() {
    let app = sdkwork_api_interface_http::gateway_router();
    let response = app
        .clone()
        .oneshot(
...
#[tokio::test]
async fn eval_run_delete_route_returns_not_found_without_local_state() {
    let app = sdkwork_api_interface_http::gateway_router();
    let response = app
        .clone()
        .oneshot(
...
#[tokio::test]
async fn eval_run_cancel_route_returns_not_found_without_local_state() {
    let app = sdkwork_api_interface_http::gateway_router();
    let response = app
        .clone()
        .oneshot(
...
#[tokio::test]
async fn eval_run_output_items_list_route_returns_invalid_request_without_local_output_state() {
    let app = sdkwork_api_interface_http::gateway_router();
    let response = app
        .clone()
        .oneshot(
            Request::builder()
                .method("GET")
                .uri("/v1/evals/eval_local_1/runs/run_local_1/output_items")
                .body(Body::empty())
                .unwrap(),
        )
        .await
    ...
    assert_openai_invalid_request(
        response,
        "Persisted local eval run output item state is required for local output item listing.",
        "invalid_eval_request",
    )
    .await;
}
...
#[tokio::test]
async fn eval_run_output_item_retrieve_route_returns_not_found_without_local_state() {
    let app = sdkwork_api_interface_http::gateway_router();
    let response = app
        .clone()
        .oneshot(
