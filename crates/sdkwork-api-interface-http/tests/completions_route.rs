...
#[tokio::test]
async fn completions_route_returns_invalid_request_without_text_generation_backend() {
    ...
    assert_eq!(
        json["error"]["message"],
        "Local completion fallback is not supported without a text generation backend."
    );
...
#[tokio::test]
async fn stateless_completions_route_relays_to_openai_compatible_provider() {
    ...
    let response = app
        .oneshot(
            Request::builder()
                ...
                .header("content-type", "application/json")
                .body(Body::from(
                    "{\"model\":\"gpt-3.5-turbo-instruct\",\"prompt\":\"relay me\"}",
                ))
                .unwrap(),
        )
        .await
...
#[tokio::test]
async fn stateful_completions_route_relays_to_openai_compatible_provider() {
    let upstream_state = UpstreamCaptureState::default();
    let listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
    let address = listener.local_addr().unwrap();
    let upstream = Router::new()
...
.header("content-type", "application/json")
                .body(Body::from(
                    "{\"external_name\":\"gpt-3.5-turbo-instruct\",\"provider_id\":\"provider-openai-official\"}",
                ))
                .unwrap(),
        )
        .await
