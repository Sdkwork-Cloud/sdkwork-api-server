...
#[tokio::test]
async fn stateless_audio_voices_route_relays_to_openai_compatible_provider() {
    let upstream_state = UpstreamCaptureState::default();
    let listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
    let address = listener.local_addr().unwrap();
    let upstream = Router::new()
        .route("/v1/audio/voices", get(upstream_audio_voices_handler))
    ...
    let json = read_json(response).await;
    assert_eq!(json["data"][0]["id"], "voice_upstream");
    assert_eq!(
...
#[tokio::test]
async fn stateful_audio_voices_route_relays_to_openai_compatible_provider() {
    let upstream_state = UpstreamCaptureState::default();
    let listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
    let address = listener.local_addr().unwrap();
    let upstream = Router::new()
...
async fn upstream_audio_voices_handler(
