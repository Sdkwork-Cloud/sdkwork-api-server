...
#[tokio::test]
async fn vector_store_search_route_returns_not_found_for_missing_local_vector_store() {
    let app = sdkwork_api_interface_http::gateway_router();
    let response = app
        .oneshot(
            Request::builder()
                .method("POST")
                .uri("/v1/vector_stores/vs_missing/search")
                .header("content-type", "application/json")
                .body(Body::from("{\"query\":\"reset password\"}"))
                .unwrap(),
        )
...
async fn stateful_vector_store_search_route_returns_not_found_without_usage() {
    ...
    let api_key = support::issue_gateway_api_key(
        ...
        "project-vector-store-search-missing",
    )
    .await;
