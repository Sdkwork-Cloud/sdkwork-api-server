...
impl SqliteAdminStore {
    ...
    pub async fn upsert_commerce_payment_attempt(
        ...
        match insert_result {
            ...
            Err(error) if sqlite_error_is_unique_violation(&error) => {
                if self
                    .find_commerce_payment_attempt(&attempt.payment_attempt_id)
                    .await?
                    .is_some()
    ...
    pub async fn list_catalog_publication_lifecycle_audit_records(
        ...
        .await?;
        rows.into_iter()
            .map(Self::map_sqlite_catalog_publication_lifecycle_audit_row)
            .collect()
    }

}

fn sqlite_error_is_unique_violation(error: &sqlx::Error) -> bool {
