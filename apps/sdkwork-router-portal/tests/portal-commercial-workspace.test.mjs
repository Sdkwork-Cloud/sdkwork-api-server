...
test('portal account services keep unsafe pricing ids sortable and matched without number coercion', () => {
  const { buildPortalAccountViewModel } = loadAccountServices();
  const now = new Date('2026-04-03T10:00:00.000Z').getTime();
  ...
  assert.equal(viewModel.commercial_posture.account_id, '9007199254740993');
  assert.equal(viewModel.commercial_posture.primary_plan_display_name, 'Workspace Retail Higher Id');
  assert.equal(viewModel.commercial_posture.primary_rate_metric_code, 'token.output');
  assert.equal(viewModel.commercial_posture.primary_rate_charge_unit, 'output_token');
  assert.equal(
    viewModel.commercial_posture.primary_rate_display_price_unit,
